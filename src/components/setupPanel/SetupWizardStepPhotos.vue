<template>
  <section class="grid gap-3.5 rounded-xl border border-amber-200/80 bg-white/58 p-4">
    ...
    <div class="grid gap-3 xl:grid-cols-[minmax(0,1.25fr)_minmax(0,0.95fr)] xl:items-start">
      <article class="grid gap-2.5 rounded-xl border border-lime-300/70 bg-white/84 p-3.5">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <p class="text-sm font-semibold text-stone-800">Upload Photos</p>
          <span class="rounded-full border border-lime-300/80 bg-lime-100/80 px-2 py-0.5 text-[11px] font-semibold text-lime-900">
            Primary Path
          </span>
        </div>
        ...
        <label class="grid gap-1.5 text-sm text-stone-700" for="photoInput">
          <span class="font-medium">Select image files</span>
          <input
            id="photoInput"
            type="file"
        ...
        <p class="text-xs text-stone-500">Accepted formats: {{ acceptedFormatsLabel }}</p>
      </article>

      <article class="grid gap-2.5 rounded-xl border border-amber-200/80 bg-white/76 p-3">
        <div class="grid gap-1">
          <p class="text-sm font-semibold text-stone-800">Load Photo Folder</p>
          <p class="text-xs text-stone-600">Useful when your references are already organized in a single folder.</p>
        </div>
    ...
    <div class="grid gap-1 rounded-lg border border-amber-200/80 bg-white/84 px-3 py-2.5 text-sm text-stone-700">
      <p>
        Current source pool:
        <span class="font-semibold text-stone-800">{{ sourcePoolCountLabel }}</span>
      </p>
      <p class="text-xs text-stone-500">{{ sourcePoolStatusHint }}</p>
    </div>
  </section>
</template>

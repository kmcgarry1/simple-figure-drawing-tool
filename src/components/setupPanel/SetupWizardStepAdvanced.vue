<template>
  <section class="grid gap-3.5 rounded-xl border border-amber-200/80 bg-white/58 p-4">
    ...
    <section class="fd-subtle-card grid gap-2 rounded-lg p-2.5">
      <button
        type="button"
        class="flex items-center justify-between gap-2 rounded-md border border-amber-200/75 bg-white/70 px-3 py-2 text-left text-sm font-semibold text-stone-800 transition-colors hover:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-300/80"
        :aria-expanded="isOpen('tagging') ? 'true' : 'false'"
        @click="toggleSection('tagging')"
      >
        <span>Photo Tags And Order</span>
        <span class="text-xs font-semibold uppercase tracking-[0.08em] text-stone-500">{{ isOpen("tagging") ? "Hide" : "Show" }}</span>
      </button>
      <div v-if="isOpen('tagging')" class="grid gap-2">
        ...
        />
        <p v-else class="text-xs text-stone-500">Add photos in Step 1 to edit order and tags.</p>
      </div>
    </section>
    ...
    <section class="fd-subtle-card grid gap-2 rounded-lg p-2.5">
      ...
      >
        ...
        <span class="text-xs font-semibold uppercase tracking-[0.08em] text-stone-500">{{ isOpen("settings") ? "Hide" : "Show" }}</span>
      </button>
    ...
    <section class="fd-subtle-card grid gap-2 rounded-lg p-2.5">
      ...
      >
        ...
        <span class="text-xs font-semibold uppercase tracking-[0.08em] text-stone-500">{{ isOpen("history") ? "Hide" : "Show" }}</span>
      </button>
